...
const { data, refresh } = await useFetch<PostDetail>(`/api/posts/${route.params.slug}`)
...
const refreshReview = async (jobId: string) => {
    try {
        const result = await $fetch<ReviewResult>(
            `/api/posts/${route.params.slug}/reviews/${jobId}`,
        )
        activeReview.value = result
        reviewLoading.value = result.job.status === 'pending' || result.job.status === 'running'
        if (reviewLoading.value)
            reviewPoll = window.setTimeout(() => void refreshReview(jobId), 1_000)
        else await refresh()
    } catch (error) {
        stopReviewPolling()
...
const reviewIsCurrent = computed(() => activeReview.value?.review?.sourceContent === state.content)
        ...
        >
            <template #body>
                ...
                <div v-else-if="activeReview?.review" class="grid gap-6">
                    <section class="grid gap-2">
                        ...
                        <p v-if="activeReview.review.summary" class="text-sm">
                            {{ activeReview.review.summary }}
                        </p>
                    ...
                    <section class="grid gap-2">
                        ...
                        <p v-if="!activeReview.review.suggestedContent" class="text-muted text-sm">
                            No text changes were proposed.
                        </p>
                        <pre
                            v-else
                            class="bg-muted max-h-72 overflow-auto rounded p-3 text-xs"
                        ><code v-for="(line, index) in diffLines" :key="index" :class="line.kind === 'added' ? 'text-success' : line.kind === 'removed' ? 'text-error line-through' : ''" class="block whitespace-pre-wrap">{{ line.kind === 'added' ? '+ ' : line.kind === 'removed' ? '- ' : '  ' }}{{ line.value }}</code></pre>
                    </section>

                    <section class="grid gap-2">
                        <h2 class="font-semibold">Review notes</h2>
                        <p class="text-sm">
                            {{ activeReview.review.notes || 'No supplemental notes.' }}
                        </p>
                    </section>

                    <UAlert
                        v-if="activeReview.review.error"
                        color="error"
                        :title="activeReview.review.error"
                    />
                    <UAlert
                        v-else-if="!reviewIsCurrent"
                        color="warning"
                        title="The editor has changed since this review. Run a new review before applying it."
                    />
                </div>
            ...
            <template #footer>
                ...
                <UButton
                    ...
                    class="ml-auto"
                    :disabled="!activeReview?.review?.suggestedContent || !reviewIsCurrent"
                    @click="acceptReview"
                />
            </template>
        </UModal>
